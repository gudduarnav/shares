...
class HoldingTotalBySymbol:
    def __init__(self, db:Database = None):
        ...
        self.selectyr = SelectFinancialYear(db=self.db)
        if self.selectyr.getYear() is None:
            return
        elif self.selectyr.getYear() <= 0:
            return


        asset = self.db.getTotalHoldingbySymbolForYear(yr=self.selectyr.getYear())
        tabledata = list()
        total_buyqty = 0
        total_buyamt = 0.0
        ...
        w = sg.Window(title="Total Holdings by Symbols for Financial Year {}".format(self.selectyr.getYear()),
